...
namespace HyperAdmin.Client.Menus
{
	...
	{
		protected virtual Func<dynamic, dynamic> ValueUpdate { get; set; }
		...
		public MenuItemSpinner( Client client, Menu owner, string label, dynamic defaultValue, dynamic min, dynamic max,
				dynamic step, bool modulus = false, string ace="", int priority = -1 ) : base( client, owner, label, ace, priority ) {
			MinValue = min;
			MaxValue = max;
			Step = step;
			_value = defaultValue;
