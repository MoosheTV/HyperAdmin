...
namespace HyperAdmin.Client.Menus
{
	public class MenuController : ClientAccessor
	{
		...
		private void HandleStandardRender( MenuItem item, float yOffset, bool isSelected ) {
			var heightOffset = yOffset - (ItemHeight / 2 - 0.005451f);
			UiHelper.DrawText( item.Label, new Vector2( MenuPosX - (MenuWidth / 2 - 0.003f), heightOffset ), isSelected ? ActiveTextColor : TextColor, 0.3f );
			if( !string.IsNullOrEmpty( item.SubLabel ) ) {
				var width = UiHelper.GetStringWidth( item.SubLabel, 0.29f, Font.ChaletLondon );
				UiHelper.DrawText( item.SubLabel, new Vector2( MenuPosX + MenuWidth / 2 - width - 0.001f, heightOffset ), isSelected ? TextColor : ActiveItemColor, 0.29f, alignment: Alignment.Left );
			}
		}
		#endregion
		...
		private async Task OnControlTick() {
			try {
				...
				// If current menu is disabled, exit to next enabled ancestor menu
				while( CurrentMenu != null && !CurrentMenu.Enabled ) {
					CurrentMenu = CurrentMenu.Parent;
				}

				// Listen for hotkeys when no menu is visible
				if( CurrentMenu == null ) {
					foreach( var kvp in _hotkeyMenus ) {
						if( !kvp.Value.Enabled || !Game.IsControlJustPressed( 2, kvp.Key ) ) continue;
